...
import { MongoTokenRepository } from "../database"
import {
  EmailRequest,
...
export class NotificationService {
  ...
  async sendPasswordMail({
    ...
    type,
    adminEmail,
  }: PasswordMailRequest): Promise<void> {
    try {
      const otp = envConfig.EMAIL_TEST_MODE
        ? envConfig.APP_TESTING_OTP
        : generateRandomOTP(constantConfig.OTP_LENGTH)

      if (!envConfig.EMAIL_TEST_MODE) {
        await this.sendEmailTemplate(
          EmailTemplateService.Instance.getTemplateRequest(type)({
            username,
            otp,
            email,
            adminEmail,
        ...
        const link = `${envConfig.SERVER_UI_URL}/account/set-password?token=${otp}&email=${encodeURIComponent(email)}`

        loggerProvider.logger.info("sendPasswordMailLink", {
          email,
          otp,
          link,
        })
  ...
  private async sendEmailTemplate({
    ...
    html,
    subject,
  }: EmailTemplateRequest): Promise<void> {
    try {
  ...
  async sendEmailWithAttachment({
    ...
    subject,
    attachments,
  }: EmailWithAttachmentRequest): Promise<void> {
    try {
