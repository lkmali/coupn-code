...
import {OrganizationConfiguration, toObjectId} from '../database'
...
export function buildExotelBasicAuthHeader(cfg: ExotelConfig | null | undefined): string | null {
  if (!cfg) return null
  const user = cfg.apiKey || cfg.customerId
  const pass = cfg.apiToken || cfg.customerSecret
  if (!user || !pass) return null
  return 'Basic ' + Buffer.from(`${user}:${pass}`).toString('base64')
}

export async function getExotelConfigForOrg(orgId: string): Promise<ExotelConfig | null> {
  if (!orgId) return null
  try {
    const row = (await OrganizationConfiguration.findOne({
      orgId: toObjectId(orgId),
    })
      .lean()
      .exec()) as any
    return row?.exotelConfiguration || null
  } catch {
    return null
  }
}
