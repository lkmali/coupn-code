...
import {
  ...
  DoctorAppointmentEmailRequest,
  PatientMeetingLinkEmailRequest,
  TemplateName,
} from '../../typings'
...
<body style="margin: 0; padding: 0; background-color: #f4f7fa; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
    ...
    try {
      switch (type) {
        ...
        case 'APPOINTMENT_BOOKED': {
          return (data: DoctorAppointmentEmailRequest): EmailTemplateRequest =>
            this.doctorAppointmentBookedTemplate(data)
        }
        case 'APPOINTMENT_RESCHEDULED': {
          return (data: DoctorAppointmentEmailRequest): EmailTemplateRequest =>
            this.doctorAppointmentRescheduledTemplate(data)
        }
        case 'APPOINTMENT_CANCELLED': {
          return (data: DoctorAppointmentEmailRequest): EmailTemplateRequest =>
            this.doctorAppointmentCancelledTemplate(data)
        }
        case 'PATIENT_MEETING_LINK': {
          return (data: PatientMeetingLinkEmailRequest): EmailTemplateRequest =>
            this.patientMeetingLinkTemplate(data)
        }
        default: {
  ...
  setPasswordTemplate({ username, otp, email }: CreateEmailTemplateRequest): EmailTemplateRequest {
    try {
      ...
      const link = `${envConfig.SERVER_UI_URL}/account/set-password/${data}?email=${email}`
  ...
  private getAppointmentStyles(borderColor: string, badgeColor: string): string {
    return `
        .info-text {
            font-size: 16px;
            color: #333;
  ...
  private getAppointmentEmailTemplate(
    ...
    const strikeStyle = options.showStrikethrough ? 'text-decoration: line-through; color: #999;' : ''
    const profileLink = data.leadId
      ? `${envConfig.SERVER_UI_URL}/app/leads?leadId=${data.leadId}`
      : data.patientId
        ? `${envConfig.SERVER_UI_URL}/app/patients?patientId=${data.patientId}`
        : ''

    const content = `
            <h2 class="greeting">Hi Dr. ${data.doctorName},</h2>
            <p class="info-text">${options.greeting}</p>

            <div class="appointment-card">
                <span class="status-badge">${options.badgeText}</span>
                <div class="field"><strong>Patient:</strong> ${data.patientName}</div>
                <div class="field"><strong>Type:</strong> ${data.appointmentType}</div>
                <div class="field" style="${strikeStyle}"><strong>Date:</strong> ${data.date}</div>
                <div class="field" style="${strikeStyle}"><strong>Time:</strong> ${data.time} - ${data.endTime}</div>
                <div class="field"><strong>Duration:</strong> ${data.duration} minutes</div>
                ${data.description ? `<div class="field"><strong>Notes:</strong> ${data.description}</div>` : ''}
                ${data.meetingLink ? `<div class="field"><strong>Meeting Link:</strong> <a href="${data.meetingLink}" style="color:#2563eb;">${data.meetingLink}</a></div>` : ''}
            </div>

            ${
              profileLink
                ? `<div style="text-align: center; margin: 25px 0;">
                <a href="${profileLink}" style="display: inline-block; background-color: #2563eb; color: #ffffff; text-decoration: none; padding: 12px 32px; border-radius: 8px; font-size: 15px; font-weight: 600;">View ${data.leadId ? 'Lead' : 'Patient'} Details</a>
            </div>`
                : ''
            }

            <p class="info-text" style="font-size:14px; color:#666;">A calendar invite (.ics) is attached to this email for your convenience.</p>

            <p class="regards" style="margin-top:20px;">Warm regards,</p>
            <p class="team-name">The AI+ Team</p>`

    return this.wrapWithEmailLayout({
      title: options.title,
      content,
      extraStyles: this.getAppointmentStyles(options.borderColor, options.badgeColor),
    })
  }

  doctorAppointmentBookedTemplate(data: DoctorAppointmentEmailRequest): EmailTemplateRequest {
    try {
      const html = this.getAppointmentEmailTemplate(data, {
        title: 'New Appointment Booked',
        greeting: 'A new appointment has been scheduled for you.',
        badgeText: 'Confirmed',
        badgeColor: '#22c55e',
        borderColor: '#2563eb',
      })
      ...
      return {
        ...
        html,
        subject: `New Appointment: ${data.patientName} on ${data.date} at ${data.time}`,
      }
    } catch (error: any) {
      loggerProvider.logger.error('doctorAppointmentBookedTemplate_Error', {
        error: error.message,
        stack: error.stack,
        email: data.doctorEmail,
      })
  ...
  doctorAppointmentRescheduledTemplate(data: DoctorAppointmentEmailRequest): EmailTemplateRequest {
    try {
      const html = this.getAppointmentEmailTemplate(data, {
        title: 'Appointment Rescheduled',
        greeting: 'An appointment has been rescheduled. Please see the updated details below.',
      ...
      return {
        email: data.doctorEmail,
        html,
        subject: `Appointment Rescheduled: ${data.patientName} - New time: ${data.date} at ${data.time}`,
      }
    } catch (error: any) {
      loggerProvider.logger.error('doctorAppointmentRescheduledTemplate_Error', {
        error: error.message,
        stack: error.stack,
        email: data.doctorEmail,
      })
  ...
  doctorAppointmentCancelledTemplate(data: DoctorAppointmentEmailRequest): EmailTemplateRequest {
    try {
      const html = this.getAppointmentEmailTemplate(data, {
        title: 'Appointment Cancelled',
        greeting: 'An appointment has been cancelled.',
        badgeText: 'Cancelled',
        badgeColor: '#ef4444',
        borderColor: '#ef4444',
      ...
      return {
        email: data.doctorEmail,
        html,
        subject: `Appointment Cancelled: ${data.patientName} on ${data.date}`,
      }
    } catch (error: any) {
      loggerProvider.logger.error('doctorAppointmentCancelledTemplate_Error', {
        error: error.message,
        stack: error.stack,
        email: data.doctorEmail,
      })
  ...
  patientMeetingLinkTemplate(data: PatientMeetingLinkEmailRequest): EmailTemplateRequest {
    try {
      const modeLabel = data.appointmentMode === 'VIDEO_CALL' ? 'Video Call' : 'Voice Call'

      const content = `
            <h2 class="greeting">Hi ${data.patientName},</h2>
            <p class="info-text">Your ${modeLabel} appointment has been scheduled.</p>

            <div class="appointment-card">
                <span class="status-badge">Confirmed</span>
                <div class="field"><strong>Type:</strong> ${data.appointmentType}</div>
                <div class="field"><strong>Mode:</strong> ${modeLabel}</div>
                <div class="field"><strong>Date:</strong> ${data.date}</div>
                <div class="field"><strong>Time:</strong> ${data.time} - ${data.endTime}</div>
                ${data.doctorName ? `<div class="field"><strong>Doctor:</strong> Dr. ${data.doctorName}</div>` : ''}
            </div>
            ...
            <p class="info-text" style="font-size:14px; color:#666;">
                Meeting Link: <a href="${data.meetingLink}" style="color:#2563eb;">${data.meetingLink}</a>
            </p>
            <p class="info-text" style="font-size:13px; color:#999;">
                Please join using the link above at your scheduled time. No software installation required.
            </p>
      ...
      return {
        ...
        html: this.wrapWithEmailLayout({
          title: `${modeLabel} Appointment Scheduled`,
          content,
          extraStyles: this.getAppointmentStyles('#2563eb', '#22c55e'),
        }),
        subject: `Your ${modeLabel} Appointment on ${data.date} at ${data.time}`,
      }
    } catch (error: any) {
      loggerProvider.logger.error('patientMeetingLinkTemplate_Error', {
        error: error.message,
        stack: error.stack,
        email: data.patientEmail,
      })
