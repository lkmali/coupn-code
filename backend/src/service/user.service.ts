...
import {
  RegisterHospitalUserDto,
  RegisterOrganizationDto,
  UserListQuery,
  UpdateUserProfileDto,
} from '../dto'
import { NotificationService } from './notification.service'
import { Transactional } from '@anantai/common'
...
export class UserService {
  ...
  private readonly doctorsRepository: MongoDoctorRepository
  ...
  constructor() {
    ...
    this.doctorsRepository = new MongoDoctorRepository()
  ...
  async verifyCredentials(credentials: UserCredentials): Promise<UserProfile> {
    try {
      ...
      const userProfile: UserProfile = {
       ...
       if (userProfile.roles.includes(Role.DOCTOR)) {
        const doctors = await this.doctorsRepository.getDoctor({ userId: toObjectId(userProfile.userId) })
        userProfile['doctorId'] = doctors?.doctorId ?? doctors?._id
      }
  ...
  async getAllUserForLead(userProfile: UserProfile): Promise<IUsers[]> {
    try {
      const users = await this.userRepository.getUserList(
        {
          orgId: toObjectId(userProfile.orgId),
          isActive:true,
          ...hasSome('roles', HospitalRole),
        },
        {
          select: { userId: 1, roles: 1, userName: 1, email: 1, lastLoginAt: 1, createdAt: 1, isActive: 1 },
        },
      )

      return users as any
    } catch (error: any) {
      loggerProvider.logger.error('getAllUserForLead_Error', {
        error: error.message,
        stack: error.stack,
  ...
  async getAllUserList(query: UserListQuery, userProfile: UserProfile): Promise<PaginateDataType<IUsers>> {
    try {
      ...
      const notAllowedRoles = [Role.PATIENT, Role.SUPER_ADMIN]
      let queryFilter: any = {
        orgId: toObjectId(userProfile.orgId),
        ...pick(query, ['isActive']),
        ...hasNone('roles', notAllowedRoles),
      }
  ...
  async registerNewUser(request: RegisterHospitalUserDto, userProfile: UserProfile) {
    try {
  ...
  private async validateRolesForUser(roles: Role[]) {
    const disallowedRoles = [Role.PATIENT, Role.SUPER_ADMIN]
  ...
  async saveNewUser(data: Omit<IUsers, 'userId'>, options: { session?: any } = {}): Promise<IUsers> {
    try {
      ...
      if (!isNil(user)) {
        // Prevent converting existing user to patient
        if (data.roles.includes(Role.PATIENT)) {
          throw badRequest(Messages.ERROR.USER_EXISTS_CANNOT_CONVERT)
        }
  ...
  private async saveValidData(
    request: RegisterHospitalUserDto,
    ...
    try {
      ...
      if (request.roles.includes(Role.DOCTOR)) {
        await this.doctorsRepository.saveDoctor(
          {
            isDelete: false,
            name: cleanUserName,
            userId: result._id,
  ...
  async activateInactiveUser(
    ...
    try {
      ...
      await this.doctorsRepository.updateDoctor(
  ...
  async updateUserProfile(
    ...
    try {
      ...
      // Sanitize once so every downstream consumer in this method (pick, doctor mirror) sees the same
      // cleaned name. Keeps the user.userName and Doctor.name strings in sync and Exotel-safe.
      ...
      if (user!.roles.includes(Role.DOCTOR) || (updateDto.roles && updateDto.roles.includes(Role.DOCTOR))) {
        const isDoctorAvailable = await this.doctorsRepository.getDoctor({
          userId: user!._id,
        })
        if (!isDoctorAvailable) {
          await this.doctorsRepository.saveDoctor(
            {
              isDelete: false,
              name: updateDto.userName??user!.userName,
              userId: user!._id,
              orgId: userProfile.orgId,
              email: user!.email,
              countryCode: '+91',
              ...
              createdBy: userProfile.userId,
              updatedBy: userProfile.userId,
              mobileNumber: updateDto.mobileNumber??user!.mobileNumber,
              specialization: updateDto.specialization ?? [],
              qualifications: updateDto.qualifications ?? [],
              experienceYears: updateDto.experienceYears ?? 0,
            },
            options,
          )
        } else {
          await this.doctorsRepository.updateDoctor(
            ...
            },
            {
              ...
              ...pick(updateDto, ['specialization', 'qualifications', 'experienceYears']),
            },
            options,
          )
        }
  ...
  async softDeleteUser(userId: UUIDTypes, userProfile: UserProfile): Promise<{ message: string }> {
    try {
      const user = await this.userRepository.getUserInformation({ _id: toObjectId(userId) })
      if (!user) throw badRequest(Messages.ERROR.USER_NOT_FOUND)

      // Check if user is trying to delete themselves
      if (String(user._id) === String(userProfile.userId)) {
        throw badRequest(Messages.ERROR.CANNOT_DELETE_SELF)
      }

      // Prevent deletion of super admin users
      if (user.roles.includes(Role.SUPER_ADMIN)) {
        throw badRequest(Messages.ERROR.CANNOT_DELETE_SUPER_ADMIN)
      }
      ...
      // Admin can only delete users from their own org
      if (!userProfile.roles.includes(Role.SUPER_ADMIN) && String(user!.orgId) !== String(userProfile.orgId)) {
        throw unauthorized(Messages.ERROR.UNAUTHORIZED)
      }

      await this.userRepository.removeUser({ _id: toObjectId(userId) })
      return { message: Messages.SUCCESS.USER_DELETED }
    } catch (error: any) {
      loggerProvider.logger.error('softDeleteUser_Error', {
        error: error.message,
        stack: error.stack,
        userId,
      })
