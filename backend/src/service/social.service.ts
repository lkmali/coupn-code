...
import {
  MongoLeadSocialRepository,
  MongoS3TempKeyRepository,
  MongoSocialContactRepository,
  MongoSocialMessageRepository,
  MongoAIErrorRepository,
  MongoMetaWebhookPayloadRepository,
  toObjectId,
} from '@anantai/common'
...
export class SocialService {
  ...
  private readonly leadSocialRepository: MongoLeadSocialRepository
  private readonly organizationConfigurationService: OrganizationConfigurationService
  private readonly redisService: RedisService
  private readonly aiErrorRepository: MongoAIErrorRepository
  private readonly metaWebhookPayloadRepository: MongoMetaWebhookPayloadRepository
  ...
  constructor() {
    ...
    this.leadSocialRepository = new MongoLeadSocialRepository()
    ...
    this.aiErrorRepository = new MongoAIErrorRepository()
    this.metaWebhookPayloadRepository = new MongoMetaWebhookPayloadRepository()
  ...
  /**
   * Process WhatsApp status webhook (sent, delivered, read, failed)
   * Updates message status in background - does not block webhook response
   * @param statuses - Array of status updates from webhook
   * @param trackingId - Tracking ID for logging
   */
  private async processWhatsAppStatusWebhook(statuses: any[], trackingId: string, orgId: string): Promise<void> {
    // Find the MetaWebhookPayload for this status webhook (saved by strategy)
    const webhookPayload = await this.metaWebhookPayloadRepository.getMetaWebhookPayload(
      { orgId: toObjectId(orgId) as any, messageType: 'statuses', processedStatus: 'received' },
      { sort: { createdAt: -1 } } as any,
    )

    let finalProcessedStatus: string = 'processed'
    ...
    for (const status of statuses) {
      ...
      switch (statusValue) {
        ...
        case 'failed':
          ...
          break
        default:
          loggerProvider.logger.info(`[${trackingId}] Unknown WhatsApp status: ${statusValue}`, {
            whatsappMessageId,
            statusValue,
          })
          continue
      ...
      // Extract error message when status is failed
      let errorMessage: string | undefined
      if (statusValue === 'failed' && status.errors && status.errors.length > 0) {
        const error = status.errors[0]
        errorMessage = `[${error.code}] ${error.title} - ${error.error_data?.details || error.message}`
        // If any status failed, mark the whole payload as failed
        finalProcessedStatus = 'failed'
        finalErrorMessage = errorMessage
      }

      loggerProvider.logger.info(`[${trackingId}] Processing WhatsApp status update`, {
        whatsappMessageId,
        status: messageStatus,
      })

      // Update social message in background - does not block
      this.socialMessageRepository.updateSocialMessage(
        { whatsappMessageId, orgId: toObjectId(orgId) } as any,
        { messageStatus, ...(errorMessage ? { errorMessage } : {}) } as any,
      )
    }

    // Update MetaWebhookPayload by _id for superadmin monitoring (once per webhook, not per status)
    if (webhookPayload?._id) {
      this.metaWebhookPayloadRepository.updateMetaWebhookPayload(
        { _id: webhookPayload._id } as any,
        {
          processedStatus: finalProcessedStatus,
  ...
  async sendMessageToAI(
    ...
    try {
      ...
      // Save error to database for empty response
      await this.aiErrorRepository.saveAIError({
        trackingId,
        agentUrl: url,
        requestBody: body,
...
Could you please try again? \u{2060}`,
      ...
      // Save error to database for exceptions
      await this.aiErrorRepository.saveAIError({
        trackingId,
        agentUrl: url,
        requestBody: body,
...
Could you please try again? \u{2060}`,
  ...
  async createLeadAndSaveMessage(
    body: Omit<ISocialContract, 'contactId'>,
    config: IOrganizationConfiguration,
    trackingId: string,
    _referral?: IWhatsAppReferral | null,
    options: { session?: any } = {},
  ): Promise<
    ISocialContract & {
  ...
  > {
    try {
      const data = await this.saveAndGetSocialContact(body, trackingId,config.phoneNumbersId[0], options)
      if (data.isBlocked) {
        return { ...data, isNewMessage: false, isFirstLanguage: false, isLanguageSelectedByUser: false }
      }
      return data
    } catch (error: any) {
      loggerProvider.logger.error(`${trackingId}_CreateLeadAndSaveMessage_Error`, {
        error: error.message,
        stack: error.stack,
        socialId: body.socialId,
        orgId: body.orgId,
    ...
    try {
      ...
      if (contractIdData) {
        ...
        if (
          ...
          body.mobileNumber =
            (await this.getMobileNumber(contractIdData.socialId, contractIdData.socialType as SocialMediaType, body.orgId)) ?? ''
  ...
  async getMobileNumber(socialId: string, source: SocialMediaType, orgId?: string): Promise<string | null> {
    try {
      if (source === SocialMediaType.WHATSAPP) {
        return socialId
      }
      const query = {
        ...(source === SocialMediaType.INSTAGRAM ? { instagramId: socialId } : {}),
        ...(source === SocialMediaType.MESSENGER ? { messengerId: socialId } : {}),
        ...(orgId ? { orgId: toObjectId(orgId) } : {}),
      }

      const result = await this.leadSocialRepository.getLeadSocial(query)
      return result ? ((result as any).whatsappId ?? null) : null
    } catch (error: any) {
      loggerProvider.logger.error('getMobileNumber_Error', {
        error: error.message,
        stack: error.stack,
        socialId,
        source,
